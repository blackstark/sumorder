...
class vm_ps_product extends vmAbstractObject {
    ...
    function validate_delete( $product_id, &$d ) {
        ...
        if( $user->gid != 25 )
        {
            $d["error"] = "ERROR: Только администратор может удалять товары" ;
            return false;
        }
    ...
    function add( &$d ) {
        ...
        $fields = array ( 'vendor_id' => $vendor_id,
            ...
            'product_nobuy' => $d['product_nobuy'],
            'product_nodiscount' => $d['product_nodiscount']);
        ...
        if( $user->gid == 25 )
        {
            ...
            if (!$ps_stock->updatevmstock) {
                $fields['product_in_stock'] = vmRequest::getInt('product_in_stock');
            }

            $fields['product_opt'] = vmRequest::getInt('product_opt');
            $fields['product_zak_price'] = vmRequest::getInt('product_zak_price');
            $fields['product_opt_rezerv'] = vmRequest::getInt('product_opt_rezerv');
            $fields['product_opt_1_pr'] = vmRequest::getInt('product_opt_1_pr');
            $fields['product_opt_2_pr'] = vmRequest::getInt('product_opt_2_pr');
            $fields['product_opt_3_pr'] = vmRequest::getInt('product_opt_3_pr');
        }
        ...
        if( $user->gid == 25 )
        {
            // обновляем цены для оптовиков

            if( vmRequest::getInt('product_opt_1') && vmRequest::getInt('product_opt_2') && vmRequest::getInt('product_opt_3') )
                $this->update_opt_price( (int)$d["product_id"], vmRequest::getInt('product_opt_1'), vmRequest::getInt('product_opt_2'), vmRequest::getInt('product_opt_3') );
        }
        ...
        if( !empty($d["related_products2"])) {
            /* Insert Pipe separated Related Product IDs */
            $related_products2 = vmGet( $d, "related_products2" );
            $q  = "INSERT INTO #__{vm}_product_relations2 ";
            $q .= "(product_id, related_products2) ";
            $q .= "VALUES ('".$d["product_id"]."','".$db->getEscaped($related_products2)."')";
            $db->setQuery($q); $db->query();
    ...
    function update( &$d ) {
        ...
        if( !empty($d["related_products2"])) {
            /* Insert Pipe separated Related Product IDs */
            $related_products2 = vmGet( $d, "related_products2" );
            $q  = "REPLACE INTO #__{vm}_product_relations2 (product_id, related_products2)";
            $q .= " VALUES( '".$d["product_id"]."', '$related_products2') ";
            $db->query($q);

        }
        else{
            $q  = "DELETE FROM #__{vm}_product_relations2 WHERE product_id='".$d["product_id"]."'";
            $db->query($q);
        }
    ...
    function get_mf_country($product_id) {
        $db = new ps_DB;

        $q = "SELECT mf_country,#__{vm}_manufacturer.manufacturer_id FROM #__{vm}_product_mf_xref,#__{vm}_manufacturer ";
        $q .= "WHERE product_id='$product_id' ";
        $q .= "AND #__{vm}_manufacturer.manufacturer_id=#__{vm}_product_mf_xref.manufacturer_id";
...
//			}
    ...
    function getProductNodiscount($product_id)
    {
        $dbpd = new ps_DB;
        $dbpdsql = 'SELECT product_nodiscount FROM jos_vm_product WHERE product_id = "'. intval($product_id) .'"';
        $dbpd->query($dbpdsql);
        $dbpd->next_record();

        if( $dbpd->f('product_nodiscount') == 'Y' )
        {
            return true;
        }
    ...
    function get_adjusted_attribute_price ($product_id, $description='') {
        ...
        if( !isset($price["product_price_nd"]) )
            $price["product_price_nd"] = $price["product_price"];
     ...
     */
    function show_price( $product_id, $hide_tax = false, $tmpl = 'price') {
        ...
        if( $auth['show_prices'] ) {
            ...
            if (isset($price_info["product_price_id"])) {
                ...
                if( $base_price_info["product_has_multiple_prices"] && !$hide_tax ) {
                    ...
                    $prices_table = "<table class=\"table table-bordered table-condensed\" id=\"table-prices\">
					  <tbody>";
                    $price_table_row = array();
                    $price_table_row['count'] = array();
                    $price_table_row['price'] = array();
                    ...
                    if ($db->num_rows()==0) {
                        ...
                        $db->query( $q );
                        while( $db->next_record() ) {
                            if( !$db->f("price_quantity_start") && !$db->f("price_quantity_end"))
                                continue;
                            $price_table_row['count'][] = $db->f("price_quantity_start")." - ".$db->f("price_quantity_end");
                            ...
                            if (!empty($my_taxrate))
                            {
                                $price_table_row['price'][] = $CURRENCY_DISPLAY->getFullValue( ($my_taxrate+1)*$db->f("product_price")*((100-$shopper_group_discount)/100) );
                            }
                            else
                                $price_table_row['price'][] = $CURRENCY_DISPLAY->getFullValue( $db->f("product_price")*((100-$shopper_group_discount)/100) );
                            //$prices_table .= "</td></tr>";
                            $i == 1 ? $i++ : $i--;
                        }
                    } else {
                        ...
                        while( $db->next_record() ) {
                            ...
                            if (!empty($my_taxrate)) {
                                $price_table_row['price'][] = $CURRENCY_DISPLAY->getFullValue( ($my_taxrate+1)*$price*((100-$shopper_group_discount)/100) );
                            }
                            else {
                                $price_table_row['price'][] = $CURRENCY_DISPLAY->getFullValue( $price*((100-$shopper_group_discount)/100) );
                            }
                            //$prices_table .= "</td></tr>";
                            $i == 1 ? $i++ : $i--;
                        }
                    ...
                    $prices_table .= "<tr>";
                    $prices_table .= "<td>Количество (шт.):</td>";
                    foreach( $price_table_row['count'] as $key => $count_val )
                    {
                        if( $key == count($price_table_row['count']) - 1)
                            $prices_table .= ("<td>". $count_val ." (коробка)</td>");
                        else
                            $prices_table .= ("<td>". $count_val ."</td>");
                    }
                    $prices_table .= "</tr>";

                    $prices_table .= "<tr>";
                    $prices_table .= "<td>Цена за шт. (руб.):</td>";
                    foreach( $price_table_row['price'] as $price_val )
                        $prices_table .= ("<td>". trim(str_replace("руб.", "", $price_val)) ."</td>");
        ...
        return $tpl->fetch( 'common/'. $tmpl .'.tpl.php');
    ...
    function update_opt_price($product_id, $opt1, $opt2, $opt3 )
    {
        //
        $db = new ps_DB;

        $db->query('SELECT product_price_id FROM jos_vm_product_price WHERE product_id = '. intval($product_id) .' AND shopper_group_id = 11');
        $db->next_record();

        $opt1id = $db->f('product_price_id');

        $db->query('SELECT product_price_id FROM jos_vm_product_price WHERE product_id = '. intval($product_id) .' AND shopper_group_id = 12');
        $db->next_record();

        $opt2id = $db->f('product_price_id');

        $db->query('SELECT product_price_id FROM jos_vm_product_price WHERE product_id = '. intval($product_id) .' AND shopper_group_id = 13');
        $db->next_record();

        $opt3id = $db->f('product_price_id');

        if( $opt1id )
        {
            $db->query('UPDATE jos_vm_product_price SET product_price = '. intval($opt1) .', mdate = '. time() .' WHERE product_price_id = '. intval($opt1id));
        }
        else
        {
            $db->query('INSERT INTO jos_vm_product_price (product_id, product_price, product_currency, cdate, mdate, shopper_group_id, product_price_vdate, product_price_edate)
						VALUES ('. $product_id .', '. intval($opt1) .', "RUB", '. time() .', '. time() .', 11, 0, 0 )');
        }

        if( $opt2id )
        {
            $db->query('UPDATE jos_vm_product_price SET product_price = '. intval($opt2) .', mdate = '. time() .' WHERE product_price_id = '. intval($opt2id));
        }
        else
        {
            $db->query('INSERT INTO jos_vm_product_price (product_id, product_price, product_currency, cdate, mdate, shopper_group_id, product_price_vdate, product_price_edate)
						VALUES ('. $product_id .', '. intval($opt2) .', "RUB", '. time() .', '. time() .', 12, 0, 0 )');
        }

        if( $opt3id )
        {
            $db->query('UPDATE jos_vm_product_price SET product_price = '. intval($opt3) .', mdate = '. time() .' WHERE product_price_id = '. intval($opt3id));
        }
        else
        {
            $db->query('INSERT INTO jos_vm_product_price (product_id, product_price, product_currency, cdate, mdate, shopper_group_id, product_price_vdate, product_price_edate)
						VALUES ('. $product_id .', '. intval($opt3) .', "RUB", '. time() .', '. time() .', 13, 0, 0 )');
        }
