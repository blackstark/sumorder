...
class vm_ps_order_change_html {
	...
	**************************************************************************/
    ...
    ?>
  			<?php
    		if($db->f('ship_method_id') == "" OR preg_match('/^standard_shipping/', $db->f('ship_method_id')) && 0) {
    		?>
      		<tr>
        		<td>
          		<form method="post" action="<?php echo $_SERVER['PHP_SELF'] ?>">
            		<select name="shipping">
              		<?php
              		$dbs = new ps_DB;
              		$q = 'SELECT shipping_rate_id, shipping_rate_name, shipping_rate_weight_start, shipping_rate_weight_end, shipping_rate_value, shipping_rate_package_fee
              				FROM #__{vm}_shipping_rate
	...
	**************************************************************************/
	...
	function list_products($product_id, $skumode=false) {
		...
		$query_list_products .= ", `jos_vm_product`.`product_id` AS prid, (SELECT SUM(pp.product_in_stock) FROM jos_vm_product AS pp WHERE pp.product_parent_id = prid) AS childcount ";
		...
		while ($db->next_record()) {
			...
			if( $db->f('childcount') && !$product_in_stock )
				$product_in_stock = $db->f('childcount');

			
			if ($skumode) {
				$display .= '>' . $db->f("product_sku") . ' (на складе: '. $product_in_stock .')</option>';
			}
			else {
				$display .= '>' . $db->f("product_name") . ' (на складе: '. $product_in_stock .') - '. $db->f("product_sku") .'</option>';
			}
		}
		$display .= '</select>';
	...
	function list_attribute($product_id, $fils=true) {
		...
		if ($fils) {
		  // Generate childlist
			$q = "SELECT product_id,product_name, product_in_stock FROM #__{vm}_product WHERE product_parent_id='$product_id'";
		}
		else {
		  // Child is selected, list siblings
			$q = "SELECT product_parent_id, product_in_stock FROM #__{vm}_product WHERE product_id='$product_id'";
			$db->setQuery($q);
			$db->query();
			$db->next_record();
        ...
        // For each child get attribute values by looping through attribute list
				...
				$display .= ' - '.$CURRENCY_DISPLAY->getFullValue($price["product_price"]) .' (на складе: '. $db->f("product_in_stock") .')';
				$display .=  '</option>';
			}
