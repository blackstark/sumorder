...
class vm_ps_order_change
{
    ...
    function change_customer_note()
    {
        ...
        // Update order
        $q = "SELECT customer_note FROM #__{vm}_orders ";
        $q .= "WHERE order_id = '" . $this->order_id . "'";
        $db->query($q);
        $prev_customer_note = $db->loadResult();
    ...
    function change_standard_shipping()
    {
        ...
        $q = "SELECT shipping_rate_name, shipping_carrier_name, shipping_rate_value, shipping_rate_value AS shipping_total FROM #__{vm}_shipping_rate, #__{vm}_shipping_carrier WHERE shipping_carrier_id = shipping_rate_carrier_id and shipping_rate_id = '" . addslashes($shipping) . "'";
        $db->query($q);
        if (!$db->next_record()) {
    ...
    function change_shipping($order_id, $shipping)
    {
        ...
        $dbo = new ps_DB();
        $q = "SELECT order_id, ship_method_id FROM #__{vm}_orders ";
        $q .= " WHERE order_id = '" . $order_id . "'";
        $dbo->query($q);
        $dbo->next_record();
        $ship_method_id = $dbo->f('ship_method_id');
        $ship_method_id_array = explode("|", $ship_method_id);

        $new_ship_method_id = '';
        for ($i = 0; $i < 3; $i++)
            $new_ship_method_id .= $ship_method_id_array[$i] . '|';

        $new_ship_method_id .= $shipping;

        $db = new ps_DB();

        $q = "SELECT order_shipping FROM #__{vm}_orders ";
        $q .= " WHERE order_id = '" . $this->order_id . "'";
        $db->query($q);
        $prev_shipping = $db->loadResult();

        $q = "UPDATE #__{vm}_orders SET ";
        $q .= "order_shipping =  '" . $shipping . "' ";

        if ($new_ship_method_id)
            $q .= ", ship_method_id =  '" . $new_ship_method_id . "' ";
    ...
    function change_discount($order_id, $discount)
    {
        ...
        $db = new ps_DB();
        $q = "SELECT order_discount FROM #__{vm}_orders ";
        $q .= "WHERE order_id = '" . $order_id . "'";
        $db->query($q);
        $prev_discount = $db->loadResult();
        ...
        if($prev_discount!=$discount){
            if ($prev_discount!=0 && $discount!=0) {
                $this->orderlog->saveLog($this->order_id, 'Изменение скидки', '', $prev_discount, $discount);
            } else {
                if ($prev_discount!=0 && $discount==0) {
                    $this->orderlog->saveLog($this->order_id, 'Удаление скидки', '', $prev_discount, $discount);
                } else {
                    $this->orderlog->saveLog($this->order_id, 'Добавление скидки', '', $prev_discount, $discount);
                }
            }
        }
    }
    ...
    function change_coupon_discount($order_id, $discount)
    {
        ...
        // Update order
        $db = new ps_DB();
        $q = "SELECT coupon_discount FROM #__{vm}_orders ";
        $q .= "WHERE order_id = '" . $order_id . "'";
        $db->query($q);
        $prev_discount = $db->loadResult();
        ...
        if($prev_discount!=$discount){
            if ($prev_discount!=0 && $discount!=0) {
                $this->orderlog->saveLog($this->order_id, 'Изменение купона на скидку', '', $prev_discount, $discount);
            } else {
                if ($prev_discount!=0 && $discount==0) {
                    $this->orderlog->saveLog($this->order_id, 'Удаление купона на скидку', '', $prev_discount, $discount);
                } else {
                    $this->orderlog->saveLog($this->order_id, 'Добавление купона на скидку', '', $prev_discount, $discount);
                }
            }
        }
    }
    ...
    function change_delete_item($order_id, $order_item_id)
    {
        ...
        $db = new ps_DB();
        $q = "SELECT product_id, product_quantity, order_status ";
...
//                exit;
        ...
        if ($scanitem['scan'] > $quantity) {
            $vmLogger->err('Не удалось удалить товар, в заказе имеются сканированые позиции');
            return -1;
        }
        ...
        if ((strtoupper($curr_order_status) != "X")) {
        ...
        $this->recalc_order($order_id);
        $this->reload_from_db = 1;
        $this->orderlog->saveLog($this->order_id, 'Удаление позиции', $product_name, $quantity, 0);
    }
    ...
    function update_nal_pay()
    {
        ...
        if (isset($_POST['nalpay']))
            $nalpay = intval($_POST['nalpay']);

        $db = new ps_DB;
        $sql = 'UPDATE jos_vm_orders SET nalpay = "' . $nalpay . '" WHERE order_id = "' . $order_id . '"';
    ...
    function update_nal_pay_arr()
    {
        $orderarr = isset($_POST['nalpayarr']) ? $_POST['nalpayarr'] : array();
        ...
        foreach ($orderarr as $key => $order_id) {
            $sql = 'UPDATE jos_vm_orders SET nalpay = "1" WHERE order_id = "' . $order_id . '"';
            $db->query($sql);
        }
    ...
    function showallbutton()
    {
        ...
        $order_id = isset($_POST['order_id']) ? $_POST['order_id'] : array();
        ...
        $text = 'Кнопки запросил ' . $auth['username'] . ' ' . date('d.m.Y H:i', time());

        $sql = 'UPDATE jos_vm_orders SET allbutton = "' . $text . '" WHERE order_id = "' . $order_id . '"';
        $db->query($sql);
