...
class vm_ps_product_attribute {
	...
	function list_attribute_drop( $product_id, $cls_suffix ) {
		...
		$db->query($q.$q1 .' ORDER BY product_name');
		...
		if( $db->num_rows() < 1 ) {
			...
			if( $product_parent_id != "0" ) {
				...
				$q = "SELECT product_id,product_name, product_in_stock FROM #__{vm}_product WHERE product_parent_id=$product_id AND product_parent_id<>0 AND product_publish='Y'" ;
				$db->setQuery( $q.$q1 ) ;
				$db->query() ;
			}
		}
		
		/*if( isset($_COOKIE['tsarev']) )
		{
			echo $q.$q1 .'<br>';
		}*/
		...
		if( $db->num_rows() > 0 ) {
			...
			$html .= "<div class=\"need_select_param\">Необходимо выбрать параметры:</div>" ;
			...
			while( $db->next_record() ) {
				...
				$product_in_stock_class = ($product_in_stock) ? '' : 'class="item_not_in_stock"';
				
				$html .= "<option ". $product_in_stock_class ." value=\"" . $db->f( "product_id" ) . "\" $selected>" ;
				...
				if( $_SESSION['auth']['show_prices'] && _SHOW_PRICES ) {
					...
					$price_parent = $ps_product->get_adjusted_attribute_price( $product_id ) ;
					...
					if( $price['product_price'] != $price_parent['product_price'] )
					{
						$delta = $price['product_price'] - $price_parent['product_price'];
						if( $delta > 0 )
							$delta = '+'. $CURRENCY_DISPLAY->getFullValue( $delta );
						else
							$delta = '-'. $CURRENCY_DISPLAY->getFullValue( $delta );
